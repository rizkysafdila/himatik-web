        ...
        <div class="col">
          <div class="card rounded-10 p-3">
            <div class="position-absolute badge bg-light mx-3 my-3">
              <a class="text-decoration-none text-dark" href="/posts?category={{ $post->category->slug }}">{{ $post->category->name }}</a>
            </div>
            <img src="https://source.unsplash.com/300x200/?{{ $post->category->name }}" class="card-img-top rounded-10" alt="{{ $post->category->name }}">
            <div class="card-body p-0 mt-3">
              <h5 class="card-title m-0">
                <a class="text-decoration-none link-dark" href="/posts/{{ $post->slug }}">
                  {{ $post->title }}
                </a>
              </h5>
              <p class="card-text mt-1"><small class="text-muted">{{ $post->created_at->diffForHumans() }}</small></p>
